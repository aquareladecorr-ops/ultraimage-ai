...
export default async function CreditsPage({
  ...
  const [{ data: profile }, { data: payments }, { data: transactions }] = await Promise.all([
    supabase.from("profiles").select("credits_available, total_credits_purchased").eq("id", userId).single(),
    supabase
      .from("payments")
      .select("id, package_name, credits_purchased, amount_brl, status, created_at, payment_method")
      .eq("user_id", userId)
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-6 md:px-10 py-12 md:py-16">
      <div className="mb-12">
        <div className="text-xs uppercase tracking-[0.3em] text-copper mb-3 font-mono">§ Créditos</div>
